...
ipcRenderer.on('form-load', (_, data) => {
    initSelectControls(data.map(x => x[3]))

    form.addEventListener('submit', event => {
        event.preventDefault();
        try {
            const type = document.getElementById('f-type').value
            const orgRow = data.find(x => x[3] === document.getElementById('f-code-org').value.toUpperCase())
            const parentRow = data.find(x => x[3] === document.getElementById('f-code-parent-org').value.toUpperCase())
            if (orgRow && (parentRow || ['2', '4'].includes(type))) {
                const data = getSubmitData(orgRow, parentRow, type)
...
ipcRenderer.on('error', (_, e) => showError(e))

// HANDLERS

const handleAddFirstSignEmployee = (e) => {
    e.preventDefault();
...
const handleAddSecondSignEmployee = (e) => {
    e.preventDefault();
...
function getSubmitData(orgRow, parentRow, type) {
    let result = {};
